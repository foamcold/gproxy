...
要修改 API 基础路径，只需在 `.env` 文件中设置 `VITE_API_STR` 即可同时为前端和后端生效：

```
# .env 文件
VITE_API_STR=/my/custom/api
```
...
- `POST {VITE_API_STR}/auth/login/access-token` - 用户登录
- `GET {VITE_API_STR}/presets/` - 获取预设列表
- `POST {VITE_API_STR}/keys/exclusive` - 生成专属密钥
